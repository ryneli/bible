...
export class VerseCursor {
  constructor(book, chapter, verse) {
    ...
    this.verseCn =
      ChiUns.books[this.book].chapters[this.chapter].verses[this.verse].text;
    this.verseEn =
  ...
  reviseVerse() {
    ...
    if (
      next >=
      ChiUns.books[this.book].chapters[this.chapter].verses[this.verse].length
    ) {
      return;
    }

    console.log(
      "VerseCursor#reviseVerse %o",
      ChiUns.books[this.book].chapters[this.chapter].verses[next].text
    );

    if (
      ChiUns.books[this.book].chapters[this.chapter].verses[next].text === null
    ) {
      this.verseEn +=
        ESV.books[this.book].chapters[this.chapter].verses[next].text;
      this.verseEnd += 1;
    } else if (
      ESV.books[this.book].chapters[this.chapter].verses[next].text === null
    ) {
      this.verseCn +=
        ChiUns.books[this.book].chapters[this.chapter].verses[next].text;
      this.verseEnd += 1;
  ...
  getVerseNumber() {
    if (this.verseEnd === this.verse) {
      ...
      return (
        this.chapter + 1 + ":" + (this.verse + 1) + "-" + (this.verseEnd + 1)
      );
    }
  }
  ...
  next() {
    if (
      ChiUns.books[this.book].chapters[this.chapter].verses.length >
      this.verseEnd + 1
    ) {
  ...
  static fromOsis(osisText) {
    ...
    return new VerseCursor(
      BibleBookIndex[bookName.toLowerCase()],
      chapter,
      verse
    );
  }
